            ...
            return this.each(function () {
                ...
                d.addQueueItem = function (j) {
                    if (b.inArray("onAddQueueItem", f.overrideEvents) < 0) {
                        ...
                        j.queueItem.find(".filename").html(m);
                    ...
                    /*if (f.fileType) {
                        if (b.isArray(f.fileType)) {
                            ...
                            for (var l = 0; l < f.fileType.length; l++) {
                                if (j.type.indexOf(f.fileType[l]) > -1) {
                                    isValidFileType = true;
                                }
                            }
                            if (!isValidFileType) {
                                d.error("FORBIDDEN_FILE_TYPE", j);
                            }
                        } else {
                            if (j.type.indexOf(f.fileType) < 0) {
                                d.error("FORBIDDEN_FILE_TYPE", j);
                            }
                        }
                    }*/
                ...
                d.checkExists = function (j) {
                    if (b.inArray("onCheck", f.overrideEvents) < 0) {
                        b.ajaxSetup({
                            async: false
                        });
                        b.post(f.checkScript, {
                            filename: j.name
                        }, function (k) {
                            j.exists = parseInt(k);
                        });
                        if (j.exists) {
                            if (!confirm("A file named " + j.name + " already exists in the upload folder.\nWould you like to replace it?")) {
                                a.cancel.call(g, j);
                                return true;
                            }
                        }
                    ...
                    if (!k.skip && !k.complete && !k.uploading) {
                        ...
                        j.onload = function (p) {
                            var s = "-------------------------" + (new Date).getTime(),
                                ...
                                r = "";
                            r += o + s + n;
                            r += 'Content-Disposition: form-data; name="Filedata[]"';
                            if (k.name) {
                                r += '; filename="' + k.name + '"';
                            }
                            r += n;
                            r += "Content-Type: application/octet-stream" + n + n;
                            r += p.target.result + n;
                            for (key in f.formData) {
                                r += o + s + n;
                                r += 'Content-Disposition: form-data; name="' + key + '"' + n + n;
                                r += f.formData[key] + n;
                            }
                            r += o + s + o + n;
                            xhr = k.xhr = new XMLHttpRequest();
                            xhr.upload.addEventListener("progress", function (t) {
                                d.progress(t, k);
                            }, false);
                            xhr.addEventListener("load", function (u) {
                                k.uploading = false;
                                var t = this.status;
                                if (t == 404) {
                                    d.error("404_FILE_NOT_FOUND", k, l);
                                } else {
                                    d.uploadComplete(u, k, l);
                                }
                            }, false);
                            var m = f.uploadScript;
                            if (f.method == "get") {
                                var q = b(f.formData).param();
                                m += q;
                            }
                            xhr.open(f.method, f.uploadScript, true);
                            xhr.setRequestHeader("Content-Type", "multipart/form-data; boundary=" + s);
