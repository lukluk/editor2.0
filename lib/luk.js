...
(function(global) {
  ...
  /**
    ...
    * @param {Function} [reviver] Method for further parsing of SVG elements, called after each fabric object created.
    */
   function loadSVGFromURL(url,xob ,callback) {
    var dat=xob;
     url = url.replace(/^\n\s*/, '').trim();
...
(function(global) {
  ...
  fabric.Object = fabric.util.createClass(/** @scope fabric.Object.prototype */ {
    ...
    transform: function(ctx) {
      ...
      var rx = this.rx*this.scaleX || 0,
          ry = this.ry*this.scaleY || 0,
    ...
    getSvgTransformline: function() {
      var angle = this.getAngle();
      var center = this.getCenterPoint();

      var NUM_FRACTION_DIGITS = fabric.Object.NUM_FRACTION_DIGITS;
      var x=Math.sin(Math.abs(angle)/180*Math.PI);
      x=Math.abs(x);
      var y=1-x;
      x=center.x-((this.height/2)*x);
      y=center.y-((this.height/2)*y);
      var translatePart = "translate(" +
                            toFixed(x, NUM_FRACTION_DIGITS) +
                            " " +
                            toFixed(y, NUM_FRACTION_DIGITS) +
                          ")";
...
getSvgTransformnoscale: function() {
      ...
      return [
        ...
        (this.scaleX === 1 && this.scaleY === 1) ? '' : (" scale(" + toFixed(1, 2) + " " + toFixed(1, 2) + ")")
      ].join('');
    },
    /**
     * @private
...
(function(global) {
  ...
  fabric.Line = fabric.util.createClass(fabric.Object, /** @scope fabric.Line.prototype */ {
    ...
    toSVG: function() {
      ...
      return [
        '<g transform="'+ this.getSvgTransformline() +'">'+'<line ',
          'x1="', this.get('x1'), '" ',
          'y1="', this.get('y1'), '" ',
          'x2="', this.get('x2'), '" ',
          'y2="', this.get('y2'), '" ',
...
(function(global) {
  ...
  fabric.Rect = fabric.util.createClass(fabric.Object, /** @scope fabric.Rect.prototype */ {
    ...
    toSVG: function() {
      ...
      return '<g transform="'+ this.getSvgTransformnoscale() +'">'+'<rect vector-effect="non-scaling-stroke" ' +
              'x="' + (-1 * (this.width*this.scaleX) / 2) + '" y="' + (-1 * (this.height*this.scaleY) / 2) + '" ' +
              'rx="' + (parseFloat(this.get('rx'))*this.scaleX) + '" ry="' + (parseFloat(this.get('ry'))*this.scaleY) + '" ' +
...
(function(global) {
   ...
   */
  fabric.Image.fromURL = function(xob,url, callback, imgOptions) {
    var dt=xob;
    var img = fabric.document.createElement('img');
    ...
    img.onload = function() {
      if (callback) {
        callback(dt,new fabric.Image(img, imgOptions));
      }
      img = img.onload = null;
    };
    img.src = url;
...
(function(global) {
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, /** @scope fabric.Text.prototype */ {
    ...
    toSVG: function() {
	 ...
	 this.src=this.src.replace('fonts/','').replace('.ttf','');
	 var ff='';
	 var ar=this.src.split('-');
	 if(ar.length>0){
	 	for(var ix in ar){
	 		if(ar[ix].indexOf('uid')>-1){
	 			ff=ar[ix];
	 		}
	 	}
	 }
	 this.src=this.src.replace(ff,'');
      return [
        '<g transform="', this.getSvgTransform(), '">',
          textAndBg.textBgRects.join(''),
          '<text ',
            (this.pdffont ? 'font-family="\'' + this.pdffont + '\'" ': ''),
