...
function dblclick() {
    $('img[type=img],img[type=svg],img[type=rect],img[type=line],img[type=circle],img[type=triangle],img[type=ellipse],span[type=text]').off('dblclick').on('dblclick', function() {
        ...
        if (type == "line") {
            var line = new fabric.Line([-50, 0, 50, 0], {
                lockScalingY: true,
                left: pleft,
                top: ptop,
                stroke: 'black',
...
function addObj(event, ui) {
    ...
    //console.log(ui.draggable,src);
    if (typeof src != 'undefined')
        if (src.indexOf('uid') > -1 && expired) {
            ...
            var srcx = ui.draggable.attr('src');
            ...
            var pdffont = ui.draggable.attr('pdffont');
    ...
    if (canvas.getActiveGroup()) {
        ...
        if (activeObject && activeObject.type === 'text') {
            ...
            $('#font option').each(function() {
                if (this.value == activeObject.fontFamily) {
                    exists = true;
                    return false;
                }
            });
            if (exists) {
                ...
                $.prompt('font not found , we will change to monofonto font', {
                    buttons: {
                        Ok: true,
                    },
                    focus: 1
                });
                activeObject.fontFamily = 'monofonto';
                $('#font').val(activeObject.fontFamily);
            }
    ...
    $.get('getFiles.php', {
        ...
        if (data != 'NO') {
            for (var i in arr) {
                ...
                res = res + '<div class="fileitem" ><div style="float:left;margin-top:6px;" ><img src="../favicon.ico" width="20px"></div>&nbsp;<div style="float:left;margin-top:6px;" class="label" >' + label + '</div ><div style="float:right;"><button class="pin" data-name="' + label + '" data-val="' + far[0] + '">PIN</button><button class="share" data-name="' + label + '" data-val="' + far[2] + '">Share</button><button class="rename" data-name="' + label + '" data-val="' + far[0] + '">Rename</button><button class="editxml" data-name="' + label + '" data-val="' + ixml + '">Open</button>' + '<button class="downloadpdf" data-val="http://fabreasy.com/editor/' + path + '">Pdf</button>' + '<button class="deletepdf" data-val="' + path + '">X</button>' + '</div></div>';
            }
        } else {
        ...
        $('#lists').html(res);
        $('.deletepdf').unbind('click').bind('click', function() {
            var t = $(this);
            var r = confirm(b3);
            if (r) {
                $.get('deletepdf.php', {
        ...
        $('.share').unbind('click').bind('click', function() {
            link = $(this).attr('data-val');
            $("#sharedlg input").val('');
            $("#sharedlg textarea").val('');
        ...
        $('#sharebtn').unbind('click').bind('click', function() {
            $.post('share.php', {
                ...
                $("#sharedlg").dialog("close");
                alert('Email sent');
            });

        });
        var pinn = '';
        $('.pin').unbind('click').bind('click', function() {
            $("#sharedlg input").val('');
            pinn = $(this).attr('data-val') + '/' + $(this).attr('data-name') + '.xml';
            $("#pindlg").dialog("open");
        });
        $('#pinbtn').unbind('click').bind('click', function() {
            $("#pindlg").dialog("close");
            $.post('changepin.php', {
                file: pinn,
                pin: $('#pin').val()
        ...
        $('.downloadpdf').unbind('click').bind('click', function() {
            var t = $(this);

            window.open($(this).attr('data-val'));
        });
        $('.editxml').unbind('click').bind('click', function() {
            $('.modal').fadeIn('slow');
